...
//
//#############################################################################
// $TI Release: CLA Math Library 4.02.01.00 $
...
#define __atan2puf32   CLAatan2PU
#endif
...
#define CLAMATH_TABLE_SIZE             64
#define CLAMATH_TABLE_SIZE_M_1         (CLAMATH_TABLE_SIZE-1)
...
static inline float32_t CLAacos_inline(float32_t fVal)
{
    ...
    xTblIdx = fabs(fVal) * CLAMATH_TABLE_SIZE_M_1;
    xTblIdx = xTblIdx * 3; 
    ...
    result = A0 + (fabs(fVal) * (A1 + (A2 * fabs(fVal))));
...
static inline float32_t CLAatan2_inline( float32_t fVal1, float32_t fVal2 )
{
    ...
    num = __mminf32(fabs(fVal1), fabs(fVal2));
    den = __mmaxf32(fabs(fVal1), fabs(fVal2));
    ...
    if((fVal1 < 0.0f) && (fVal2 < 0.0f) && (fabs(fVal1) < fabs(fVal2)))
    {
        result = result - CLAMATH_PI;
    }
    else if((fVal1 < 0.0f) && (fVal2 < 0.0f) && (fabs(fVal1) >= fabs(fVal2)))
    {
...
static inline float32_t CLAisqrt_inline(float32_t fVal)
{
    ...
    Yee = Ye * (1.5 - Ye * Ye * fVal * 0.5);  //Newton rapshon iteration 1
    Yeee = Yee * (1.5 - Yee * Yee * fVal * 0.5); //Newton rapshon iteration 2
