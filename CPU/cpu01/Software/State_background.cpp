...
void Background_class::init()
{
    ...
    NV_CLASS_read(sec_settings);

    NV_CLASS_read(sec_CT_characteristic);
    NV_CLASS_read(sec_H_settings);
    NV_CLASS_read(sec_calibration_data);
    NV_CLASS_read(sec_meter_data);
...
//        master_slave_selector = 1;
    ...
    if(control_ACDC.triggers.bit.SD_save_H_settings)
    {
        ...
        if(!NV_CLASS_save(sec_H_settings)) status_ACDC.SD_no_harmonic_settings = 0;
    ...
    if(control_ACDC.triggers.bit.SD_save_settings)
    {
        ...
        if(!NV_CLASS_save(sec_settings)) status_ACDC.SD_no_settings = 0;
