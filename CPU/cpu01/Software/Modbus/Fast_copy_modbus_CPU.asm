...
_Fast_copy_modbus_CPUasm:
		...
		.asg 	0 , x
		.loop 	FPGA_KALMAN_STATES
		MOV32	*(#_Modbus_Converter.input_registers.harmonic_rms_values.I_grid_a + 2*(x + FPGA_KALMAN_STATES*y)), *XAR0++
		.eval 	x+1, x
		.endloop
		.asg 	0 , x
		.loop 	FPGA_KALMAN_STATES
		MOV32	*(#_Modbus_Converter.input_registers.harmonic_THD_individual.I_grid_a + 2*(x + FPGA_KALMAN_STATES*y)), *XAR0++
		.eval 	x+1, x
		.endloop
		...
		.asg 	0 , x
		.loop 	FPGA_KALMAN_STATES
		MOV32	*(#_Modbus_Converter.input_registers.harmonic_rms_values.U_grid_a + 2*(x + FPGA_KALMAN_STATES*y)), *XAR0++
		.eval 	x+1, x
		.endloop
		.asg 	0 , x
		.loop 	FPGA_KALMAN_STATES
		MOV32	*(#_Modbus_Converter.input_registers.harmonic_THD_individual.U_grid_a + 2*(x + FPGA_KALMAN_STATES*y)), *XAR0++
		.eval 	x+1, x
		.endloop
